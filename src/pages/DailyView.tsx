...
export function DailyView({ setView }: DailyViewProps) {
  const [salahTracker, setSalahTracker] = useLocalStorage("dailySalahTracker", {
  ...
  useEffect(() => {
    if (salahTracker.date !== new Date().toDateString()) {
      setSalahTracker({
        date: new Date().toDateString(),
        status: {
          Fajr: false,
  ...
  const toggleSalah = (prayer: string) => {
    setSalahTracker((prev) => ({
      ...prev,
      status: { ...prev.status, [prayer]: !prev.status[prayer] },
    }));
  };
      ...
      <div className="flex flex-col gap-6 overflow-y-auto pb-10">
        ...
        <div className="bg-white rounded-[24px] overflow-hidden shadow-lg border border-slate-100 flex flex-col p-5">
          <div className="flex items-center justify-between mb-4 border-b border-slate-50 pb-3">
            ...
            <span className="text-[12px] font-bold text-[#7da65e] bg-[#edf6e9] px-2.5 py-1 rounded-full uppercase tracking-wider">
              Tracker
            </span>
          </div>
          <div className="flex items-center gap-2 justify-between">
            {["Fajr", "Dhuhr", "Asr", "Maghrib", "Isha"].map((prayer) => {
              const isCompleted = salahTracker.status[prayer];
              return (
                <button
                  key={prayer}
                ...
                >
                  <div
                    className={`w-[3.25rem] h-[3.25rem] rounded-[18px] flex items-center justify-center transition-colors ${isCompleted ? "bg-[#7da65e] text-white shadow-sm" : "bg-slate-50 text-slate-400 group-hover:bg-slate-100 border border-slate-100"}`}
                  >
                    {isCompleted ? (
                  ...
                  <span
                    className={`text-[12px] font-bold ${isCompleted ? "text-[#7da65e]" : "text-slate-500"}`}
                  >
                    {prayer}
                  </span>
                </button>
        ...
        <div className="bg-white rounded-[24px] overflow-hidden shadow-lg border border-slate-100 flex flex-col">
          ...
          <div className="px-5 py-4 flex items-center justify-between border-b border-slate-50">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col leading-tight">
                <span className="text-[26px] font-extrabold text-slate-800 tracking-tight leading-none mb-0.5">
                  Ayat
                </span>
                <span className="text-[12px] font-bold text-slate-900 tracking-wider">
                  OF THE DAY
                </span>
              </div>
            </div>
            <button
              onClick={() =>
                handleShare(
                  "Ayat of the Day",
                  "Check out this Ayat of the Day from the app!",
                )
              }
          ...
          <div className="relative w-full h-[400px] overflow-hidden bg-[#dabd9b] flex items-center justify-center">
            ...
            <div className="relative w-[90%] h-[90%] bg-gradient-to-b from-[#704627] to-[#8d5b35] rounded-t-full shadow-2xl z-20 flex flex-col items-center pt-10 px-4 text-center border-t-2 border-x-2 border-white/10">
              <div className="text-white drop-shadow-md pb-4 border-b border-white/20 w-[80%] mb-4">
                <p
                  className="text-[28px] font-bold leading-normal text-[#fdefdc]"
                  dir="rtl"
                >
                  وَ اَنْزَلَ اللّٰهُ عَلَيْكَ الْكِتٰبَ وَ الْحِكْمَةَ وَ
                  عَلَّمَكَ مَا لَمْ تَكُنْ تَعْلَمُ
                </p>
                <p className="text-[16px] mt-2 font-medium text-[#e3c7a6]">
                  (النساء:113)
                </p>
              </div>
              <p className="text-[14px] font-bold text-[#23150d] mb-2 z-30 bg-[#c79c6d] px-3 py-1 rounded-full shadow-sm drop-shadow-sm">
                ترجمہ کنزالعرفان
              </p>
              <p
                className="text-[18px] font-medium leading-relaxed text-white drop-shadow-md z-30"
                dir="rtl"
              >
                اور اللہ نے آپ پر کتاب اور حکمت نازل فرمائی اور آپ کو وہ سب کچھ
                سکھا دیا جو آپ نہ جانتے تھے۔
              </p>
            </div>
          </div>
          ...
          <div className="px-5 py-4 bg-white">
            <button
              ...
              className="text-[#bf5c5c] font-medium text-[15px] hover:underline"
            >
              Read more
            </button>
          </div>
        ...
        <div className="bg-white rounded-[24px] overflow-hidden shadow-lg border border-slate-100 flex flex-col">
          ...
          <div className="px-5 py-4 flex items-center justify-between border-b border-slate-50">
            ...
            <button
              onClick={() =>
                handleShare(
                  "Hadees of the Day",
                  "Check out this Hadees of the Day from the app!",
                )
              }
              className="p-2 hover:bg-slate-50 rounded-full transition-colors"
            >
          ...
          <div className="relative w-full h-[500px] overflow-hidden bg-slate-100 flex flex-col">
            <img
              src="https://images.unsplash.com/photo-1565552645632-d725e8bfc19a?auto=format&fit=crop&q=80&w=600"
              alt="Kaaba"
              className="absolute right-0 top-0 w-[60%] h-full object-cover opacity-90 object-right"
            />
            ...
            <div className="relative z-20 w-[65%] h-full bg-white/40 backdrop-blur-[2px] p-4 flex flex-col items-center justify-center text-center shadow-[4px_0_24px_rgba(0,0,0,0.05)] border-r border-white/50 rounded-r-[3rem] my-4 ml-2">
              <div className="border border-[#758469]/30 rounded-t-full rounded-b-xl p-4 h-full flex flex-col items-center justify-center bg-white/60">
                <h3
                  className="text-[24px] font-bold text-[#006e89] mb-4 drop-shadow-sm font-serif"
                  dir="rtl"
                >
                  فرمانِ مصطفیٰ{" "}
                  <span className="text-[12px] text-slate-500">ﷺ</span>
                </h3>
                <p
                  className="text-[18px] font-bold text-[#9d5c2e] mb-2"
                  dir="rtl"
                >
                  دو چتکبرے
                </p>
                <p
                  className="text-[15px] leading-relaxed text-[#166534] font-medium"
                  dir="rtl"
                >
                  رسول اللہ صلی اللہ علیہ وسلم نے سینگ والے بکروں کی قربانی کی
                  کہ انہیں اپنے ہاتھ سے{" "}
                  <span className="font-bold text-[#c7823f]">ذبح</span> کیا، بسم
                  اللہ و تکبیر کہی، فرمایا کہ میں نے آپ کو ان بکروں کی کروٹوں پر
                  اپنا <span className="font-bold text-[#c7823f]">قدم</span>{" "}
                  رکھے دیکھا، آپ فرماتے تھے:{" "}
                  <span className="text-[18px] font-bold block mt-2 text-[#7f5130]">
                    بسم اللہ واللہ اکبر۔
                  </span>
                </p>
                <p
                  className="text-[10px] mt-4 text-slate-600 font-medium"
                  dir="rtl"
                >
        ...
        <div className="bg-white rounded-[24px] overflow-hidden shadow-lg border border-slate-100 flex flex-col">
          ...
          <div className="px-5 py-4 flex items-center justify-between border-b border-slate-50">
            ...
            <button
              onClick={() =>
                handleShare(
                  "Fatwa of the Day",
                  "Check out this Fatwa of the Day from the app!",
                )
              }
              className="p-2 hover:bg-slate-50 rounded-full transition-colors"
            >
          ...
          <div className="relative w-full h-[350px] overflow-hidden bg-gradient-to-b from-[#e9dec7] to-[#fac28d] flex items-center justify-center flex-col p-6 text-center">
            ...
            <div className="relative z-20 flex flex-col items-center drop-shadow-xl h-full justify-start pt-6">
              <h3
                className="text-[32px] font-extrabold text-[#472d1a]"
                dir="rtl"
              >
              ...
              <h4 className="text-[36px] font-black text-[#5c2b18] mt-2 mb-2">
                12 ذوالحجہ
                <span className="text-[26px] text-[#472d1a] font-bold">
                  {" "}
                  کو سفر پر ہو تو
                </span>
              </h4>
              <h5 className="text-[28px] font-bold text-[#472d1a] mt-1">
                قربانی کا حکم
              </h5>
            </div>
          ...
          <div className="px-5 py-4 bg-white">
            <button
              onClick={() =>
                window.open("https://daruliftaahlesunnat.net/", "_blank")
              }
              className="text-[#bf5c5c] font-medium text-[15px] hover:underline"
            >
              Read more

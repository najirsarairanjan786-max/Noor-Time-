...
interface CalendarDayInfo {
  ...
  hijri: { day: string; date: string; month: { en: string; ar: string }; year: string; holidays: string[] };
}
...
export function CalendarView() {
  ...
  const [selectedEvent, setSelectedEvent] = useState<{date: string, events: string[]} | null>(null);
    ...
    async function fetchMonth() {
      ...
      try {
        ...
        const fetchReq = (d: Date) => 
          fetch(`https://api.aladhan.com/v1/gToHCalendar/${d.getMonth()+1}/${d.getFullYear()}`).then(r => r.json());
        
        const [prevData, currData, nextData] = await Promise.all([
          fetchReq(prev), fetchReq(currentDate), fetchReq(next)
        ]);
        ...
        const adjustedCalendar = currData.data.map((day: any, idx: number) => {
          ...
          const mappedHijri = flatData[flatIdx] ? flatData[flatIdx].hijri : day.hijri;
          return {
            ...day,
            hijri: mappedHijri
          };
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-8 bg-pink-900/60 backdrop-blur-lg border border-pink-500/20 shadow-2xl rounded-2xl p-4">
        <button onClick={prevMonth} className="p-2 bg-pink-800/40 rounded-full hover:bg-pink-700/50 transition">
        ...
        <div className="text-center">
          ...
          {calendarData.length > 0 && (
            <p className="text-sm font-serif text-pink-300 mt-1">
              {calendarData[0].hijri.month.en} / {calendarData[calendarData.length - 1].hijri.month.en} {calendarData[0].hijri.year} AH
            </p>
          )}
        </div>
        <button onClick={nextMonth} className="p-2 bg-pink-800/40 rounded-full hover:bg-pink-700/50 transition">
      ...
      <div className="bg-pink-900/60 backdrop-blur-lg border border-pink-500/20 shadow-2xl rounded-2xl p-4 overflow-hidden relative">
        ...
        <div className="grid grid-cols-7 gap-1 md:gap-1.5">
          ...
          {calendarData.map((day, idx) => {
            ...
            const arabicHijriDay = day.hijri.day.toString().split('').map(d => '٠١٢٣٤٥٦٧٨٩'[parseInt(d)]).join('');

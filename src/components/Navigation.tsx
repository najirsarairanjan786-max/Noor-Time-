...
export function Navigation({
  ...
  const [localProfile] = useLocalStorage("islamic-app-local-profile", { firstName: "", lastName: "", photoURL: "" } as any);
  const profileName = localProfile?.firstName
  ...
  const TABS = [
    ...
    { id: "profile", icon: UserCircle, label: profileName },
  ];

  const activeIndex =
    TABS.findIndex((t) => t.id === view) === -1
      ? 0
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none rounded-t-[2rem] shadow-[0_-5px_40px_rgba(0,0,0,0.1)] bg-transparent">
          ...
          >
            ...
            <div className="absolute top-0 bottom-0 left-[calc(50%-36px)] w-[72px]">
              ...
              >
                <path
                  d="M0,0 C18,0 18,34 36,34 C54,34 54,0 72,0 V36 H0 Z"
                  fill="white"
                />
              </svg>
          ...
          {TABS.map((tab, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  {tab.id === "profile" && (localProfile?.photoURL || user?.photoURL) ? (
                    <div
                      className={`w-6 h-6 rounded-full overflow-hidden border ${isActive ? "border-white" : "border-slate-800"}`}
                    >
                      <img
                  ...
                  <AnimatePresence>
                    {isActive && (
                      <motion.span 
                        ...
                        className="absolute text-[10px] font-bold text-[#c83271] whitespace-nowrap drop-shadow-sm tracking-wide"
                      >
                        {tab.label}
                      </motion.span>
                    )}

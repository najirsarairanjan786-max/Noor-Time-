...
apt-get install -y -qq -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" openjdk-17-jdk-headless unzip wget curl
...
chmod +x gradlew
export GRADLE_OPTS="-Dorg.gradle.jvmargs='-Xmx1024m -XX:MaxMetaspaceSize=512m' -Dorg.gradle.daemon=false"
./gradlew assembleDebug --no-daemon --max-workers=1 > /tmp/build_5.log 2>&1
echo "Build finished."
